...
public class PieChartRenderer: ChartDataRendererBase
{
    ...
    public override func drawHighlighted(context context: CGContext, indices: [ChartHighlight])
    {
        ...
        for i in 0 ..< indices.count
        {
            ...
            let translationAngle: CGFloat
            if (xIndex == 0){
                translationAngle = 0.5 * (90 + (absoluteAngles[xIndex] - 90)) - (absoluteAngles[xIndex] - 90)
            }
            else {
                translationAngle = -0.5 * (absoluteAngles[xIndex] - 90) - 0.5 * (absoluteAngles[xIndex - 1] - 90)
            }
            var translationX = 7 * cos(translationAngle * 3.14/180.0)
            var translationY = -7 * sin(translationAngle * 3.14/180.0)
            center.x = center.x + translationX
            center.y = center.y + translationY
